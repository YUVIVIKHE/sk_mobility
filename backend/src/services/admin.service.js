...
const createUser = async (data, req) => {
  ...
  const [result] = await db.query(
    ...
    [data.roleId, data.email, passwordHash, data.firstName, data.lastName, data.phone, data.isActive ?? 1, data.isVerified ?? 0]
  );
...
module.exports = {
  listUsers, createUser, listRoles, listPermissions,
